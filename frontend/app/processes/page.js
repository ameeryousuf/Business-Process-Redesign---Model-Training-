...
import { getUser, clearSession, fetchProcesses, fetchProcessBundle } from "@/lib/api";
...
export default function ProcessesPage() {
  ...
  const load = useCallback(async (targetPage, targetSearch) => {
    setLoading(true);
    setError(null);
    try {
      const result = await fetchProcesses({ page: targetPage, limit: PAGE_SIZE, search: targetSearch });
  ...
  useEffect(() => {
    if (!authed) return;
    load(page, search);
  }, [authed, page, search, load]);

  // Debounce the raw keystrokes into `search` so typing feels real-time without
  // firing a request per keystroke; resets to page 1 whenever the query changes.
  useEffect(() => {
      ...
      <div className="px-6 md:px-16 py-10 space-y-6 max-w-6xl mx-auto">
        <div className="relative flex items-center">
          <input
            ...
            placeholder="Search by process name or code…"
        ...
        {!loading && processes.length > 0 && (
          <div className="flex items-center justify-between text-sm text-gray-500">
            <span>
              Page {page} of {totalPages} · {total} processes
            </span>
