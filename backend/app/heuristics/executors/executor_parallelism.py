...
def apply_parallelism(parsed, task_a_id, task_b_id):
    ...
    if task_a.get("parallel_group") or task_b.get("parallel_group"):
        return None
    ...
    original_incoming_probability = incoming_flow["probability"]
    original_outgoing_probability = outgoing_flow["probability"]

    flows.remove(incoming_flow)
    flows.remove(middle_flow)
    flows.remove(outgoing_flow)

    flows.append({"id": f"{incoming_flow['id']}_new", "source": incoming_flow["source"], "target": split_id, "probability": original_incoming_probability})
    ...
    flows.append({"id": f"{outgoing_flow['id']}_new", "source": join_id, "target": outgoing_flow["target"], "probability": original_outgoing_probability})
